...
class Generator(nn.Module):
    ...
    def __init__(self, **kargs):
        ...
        for i, stride in enumerate(kargs['stride_list']):
            ...
            for j in range(kargs['num_blocks']):
                self.incoming_channels.append(ngf)
    ...
    def forward(self, input):
        ...
        from deblurgan.utils import deprocess_image, preprocess_image_7760
        ...
        l = len(self.head_layers)
        old_layers = nn.ModuleList()
        incoming_shapes = []
        # self.incoming_channels.insert(0, 26)
        incoming_shapes.append((output.shape[2], output.shape[3]))
        old_layers.extend(self.layers)
        for idx, (layer, head_layer) in enumerate(zip(self.layers, self.head_layers)):
            output = layer(output)
            incoming_shapes.append((output.shape[2], output.shape[3])) 
            if idx == l-1:
                g = generator_model()
                g.load_weights('generator_3_1804.h5')
                ...
                output_tf = np.squeeze(np.array(tf.convert_to_tensor(output.cpu().numpy())) * 255.0)
                ...
                out_list.append(generated_images)
            elif idx != l-1:
                old_channel = output.shape[1]
                downer = nn.Conv2d(old_channel, 3, kernel_size=(1, 1), stride=(1, 1), device='cpu')
                output = downer(output)
                output = transforms.Resize((256, 256))(output)
                output = torch.permute(output, (0, 2, 3, 1))
                output_tf = np.array(tf.convert_to_tensor(output.cpu().numpy()))
                g = generator_model()
                g.load_weights('generator_3_1804.h5')
                output_tf = g.predict(x=output_tf, batch_size=1)
                output = torch.tensor(output_tf)
                output = torch.permute(output, (0, 3, 1, 2))
                for sid, redo_layer in enumerate(old_layers[:idx+1]):
                    output = nn.Conv2d(output.shape[1], self.incoming_channels[sid], kernel_size=(1,1), stride=(1, 1), device='cpu')(output)
                    output = torch.sigmoid(img_out) if self.sigmoid else (torch.tanh(img_out) + 1) * 0.5
                    output = transforms.Resize(incoming_shapes[sid])(output)
                    output = redo_layer(output)
        return  out_list
